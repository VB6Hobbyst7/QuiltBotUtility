...
{
    public abstract class DefaultMoneyManager
    {
        ...
        protected DefaultMoneyManager(IAccount account, Algo algo)
        {
        ...
        protected bool ValidateVolume(double price, double stopLoss, double volume)
        {
            ...
            todayLoss += thisLoss;
            todayLoss += algo.History.Where(c => c.EntryTime >= DateTime.UtcNow.Date).Where(c => c.ClosingDealId > 0)
                .Sum(c => c.NetProfit);


            if (todayLoss >= OneDayMaxLoss)
                return false;
